...
describe('cssMinifier functions', () => {
  ...
  beforeEach((done) => {
    workspace
      .openTextDocument(
        Uri.file(path.resolve(__dirname, './fixture/style.css')),
      )
      .then(
        (doc) => {
  ...
  function checkAssert(expected) {
    minifier.cssContent.forEach((content, index) => {
      console.log(`OR: ${content} ==  DE: ${expected[index]} => ${content === expected[index]}`);
  ...
  xit('Replaces zero prefix from float values', (done) => {
    workingOnSpace.then(
      () => {
        if (window.activeTextEditor.document.languageId === LANGUAGE) {
  ...
  xit('Replaces zero units by zero', (done) => {
    workingOnSpace.then(
      () => {
        if (window.activeTextEditor.document.languageId === LANGUAGE) {
  ...
  xit('Removes single and double quotes', (done) => {
    workingOnSpace.then(
      () => {
        if (window.activeTextEditor.document.languageId === LANGUAGE) {
  ...
  xit('Minify the css file', (done) => {
    workingOnSpace.then(
      () => {
        if (window.activeTextEditor.document.languageId === LANGUAGE) {
          minifier.setContent(
            window.activeTextEditor.document.getText().split('\n'),
          );
          minifier.replaceFullHexadecimalByShorten();
          minifier.cleanSpace();
          minifier.cleanUnitsZeroValue();
  ...
  xdescribe('Reported issues', () => {
    it('wrong minifying :hover when appends :not() #12', (done) => {
      ...
      const expect =
        'header nav ul li:not(.nohover):hover ul{}div.guide p:not(.guideintro):not(.nonumber):nth-child(even):before,div.guide p:not(.guideintro):not(.nonumber):nth-child(odd):before{}';
